 ...
 */
class SQLXGBoostRegressor(override val uid: String) extends SQLAlg with MllibFunctions with Functions with BaseRegression {
  ...
  override def train(df: DataFrame, path: String, params: Map[String, String]): DataFrame = {
    ...
    trainModelsWithMultiParamGroupOrCrossValidator(df, path, params, () => {
      getXGBoostRegressor(params)
    }, (_model, fitParam) => {
      evaluateTable match {
        case Some(etable) =>
          val model = _model.asInstanceOf[XGBoostRegressionModel]
  ...
  override def batchPredict(df: DataFrame, path: String, params: Map[String, String]): DataFrame = {
    val model = load(df.sparkSession, path, params).asInstanceOf[ArrayBuffer[XGBoostRegressionModel]].head
    model.transform(df)
  }

  override def explainParams(sparkSession: SparkSession): DataFrame = {
    _explainParams(sparkSession, () => {
      new XGBoostRegressor(Map("tracker_conf" -> TrackerConf(0L, "scala")))
    })
  }
  ...
  override def load(sparkSession: SparkSession, path: String, params: Map[String, String]): Any = {
    ...
    val model = XGBoostRegressionModel.load(bestModelPath(0))
    ArrayBuffer(model)
  }


  override def explainModel(sparkSession: SparkSession, path: String, params: Map[String, String]): DataFrame = {
    val models = load(sparkSession, path, params).asInstanceOf[ArrayBuffer[XGBoostRegressionModel]]
  ...
  override def predict(sparkSession: SparkSession, _model: Any, name: String, params: Map[String, String]): UserDefinedFunction = {
    val model = sparkSession.sparkContext.broadcast(_model.asInstanceOf[ArrayBuffer[XGBoostRegressionModel]](0))
    val f = (vec: Vector) => {
  ...
  override def codeExample: Code = Code(SQLCode, CodeExampleText.jsonStr +
    """
      ...
      |as data1;
      |-- 使用XGBoost算法
      |train data1 as XGBoostRegressor.`/tmp/model` where
      |-- 设置为true,可以保留训练的模型版本
      |keepVersion="true"
      |-- 用于评估的数据集
   ...
   * @return
   */
  def getXGBoostRegressor(params: Map[String, String]): Params = {

    var modelParams: Params = new XGBoostRegressor(Map(
      "tracker_conf" -> TrackerConf(0L, "scala")
    ))

    if (cvEnabled) {
      val booster = modelParams.asInstanceOf[XGBoostRegressor]
   ...
   * @return
   */
  def getBestXGBoostModel(alg: Params, trainData: DataFrame, fitParam: Map[String, String]): XGBoostRegressionModel = {

    val cvModel: CrossValidator = alg.asInstanceOf[CrossValidator]
    val booster = cvModel.getEstimator.asInstanceOf[XGBoostRegressor]
    ...
    if (fitParam.isDefinedAt("cv_evaluator")) {
      evaluator.setMetricName(fitParam.getOrElse("cv_evaluator", "rmse"))
    }
    ...
    val model = cvm.asInstanceOf[CrossValidatorModel].bestModel.asInstanceOf[XGBoostRegressionModel]
  ...
  def trainModelsWithMultiParamGroupOrCrossValidator(df: DataFrame, path: String, params: Map[String, String],
    ...
    val mf = (trainData: DataFrame, fitParam: Map[String, String], modelIndex: Int) => {
      ...
      var model: XGBoostRegressionModel = null
      try {
        if (cvEnabled) {
          model = getBestXGBoostModel(alg, trainData, fitParam)
        } else {
          model = alg.asInstanceOf[XGBoostRegressor].fit(trainData)
        }
