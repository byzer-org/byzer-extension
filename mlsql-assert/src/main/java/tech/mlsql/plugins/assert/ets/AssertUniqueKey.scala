...
class AssertUniqueKey(override val uid: String) extends SQLAlg
    ...
    args match {
      case List(tableName, columns) =>
        ...
        if (validateResult.length > 0) {
          session.createDataFrame(session.sparkContext.parallelize(validateResult.map(Row(_))), StructType(Seq(StructField("ConflictKey", StringType))))
        } else {
